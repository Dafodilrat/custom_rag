from rag.database import VectorDBManager
from rag.database import VectorDBConfig
import logging
...
def main():
    ...
    try:
        ...
        # Create config using the new from_args class method
        config = VectorDBConfig.from_args(db_path=VECTOR_DB_PATH, workspace_path=PDF_FOLDER_PATH)
        vector_db_manager = VectorDBManager(config=config)
    ...
    query = "what is the robotics arm project about?"
